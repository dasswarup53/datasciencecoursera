best<-function(state,outcome){
  setwd("/home/swarupdas/data_science/hospitaldata/")
  data<-read.csv("outcome-of-care-measures.csv",colClasses ="character")
  state_hospital<-data[data$State==state,]
  if(nrow(state_hospital)==0)
  {
    stop("Invalid state")
  }
  if(outcome=="heart attack")
  {
    df<-state_hospital[which.min(state_hospital[,11]),]
    print(df$Hospital.Name)
    
  }
  if(outcome=="heart failure")
  {
    df<-state_hospital[which.min(state_hospital[,17]),]
    print(df$Hospital.Name)
  }
  if(outcome=="pneumonia")
  {
    df<-state_hospital[which.min(state_hospital[,23]),]
    print(df$Hospital.Name)
  }
